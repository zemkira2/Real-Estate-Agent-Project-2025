...
export default function DashboardContent({ mode }: Props) {
  const router = useRouter();
  const [user, setUser] = useState<User | null>(null);
  const [loading, setLoading] = useState(true);
  const [showPrefsModal, setShowPrefsModal] = useState(false);

  // Search form state — initialised from preferences after auth check
  const [budgetMin, setBudgetMin] = useState(400000);
  const [budgetMax, setBudgetMax] = useState(1200000);
  ...
  useEffect(() => {
    async function checkAuth() {
      try {
        ...
        setUser(u);
        window.localStorage.setItem(LOCAL_AUTH_USER_KEY, JSON.stringify(u));

        // Pre-populate form from saved preferences
        if (u.preferences) {
          setBudgetMin(u.preferences.budgetMin);
          setBudgetMax(u.preferences.budgetMax);
          setPropertyType(u.preferences.propertyType);
          setSelectedSuburbs(u.preferences.suburbs ?? []);
          setMinBedrooms(u.preferences.minBedrooms);
        }
      } catch {
        window.localStorage.removeItem(LOCAL_AUTH_USER_KEY);
        router.push("/login");
  ...
  const purpose = mode; // locked to page mode
  ...
  const modeLabel = mode === "live" ? "For Living" : "For Investment";
  const modeColor = mode === "live" ? "bg-emerald-100 text-emerald-700" : "bg-gold-100 text-gold-700";
      ...
      {showPrefsModal && (
        <OnboardingModal
          ...
          onComplete={(prefs) => {
            ...
            setPropertyType(prefs.propertyType);
            setSelectedSuburbs(prefs.suburbs);
            setMinBedrooms(prefs.minBedrooms);
        ...
        <div className="grid lg:grid-cols-[280px_1fr] gap-7">
          ...
          <div>
            <div className="bg-white rounded-2xl border border-stone-200/70 shadow-card p-6 sticky top-6">
              ...
              {/* Mode badge */}
              <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider mb-4 ${modeColor}`}>
                {mode === "live" ? "🏡" : "📈"} {modeLabel}
              </div>
              ...
              <div className="mb-4">
                ...
                <div className="relative">
                  ...
                  <input
                    type="number" value={budgetMin} onChange={(e) => setBudgetMin(Number(e.target.value))}
                    min={0} step={10000}
              ...
              <div className="mb-5">
                ...
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-stone-400 text-sm">$</span>
                  <input
                    type="number" value={budgetMax} onChange={(e) => setBudgetMax(Number(e.target.value))}
                    min={0} step={10000}
                    className="w-full pl-7 pr-3 py-2.5 bg-stone-50 border border-stone-200 rounded-xl text-sm text-primary-900 focus:outline-none focus:ring-2 focus:ring-primary-900/10 focus:border-primary-900/30 transition-all"
                  />
                </div>
              ...
              <div className="mb-5">
                ...
                <select
                  value={minBedrooms} onChange={(e) => setMinBedrooms(Number(e.target.value))}
                  className="w-full px-3 py-2.5 bg-stone-50 border border-stone-200 rounded-xl text-sm text-primary-900 focus:outline-none focus:ring-2 focus:ring-primary-900/10 focus:border-primary-900/30 transition-all appearance-none cursor-pointer"
                >
              ...
              <div className="mb-5">
                <div className="flex items-center justify-between mb-2">
                  <p className="text-[10px] font-semibold uppercase tracking-wider text-stone-400">Suburbs</p>
                  {selectedSuburbs.length > 0 && (
                    <button onClick={() => setSelectedSuburbs([])}
                ...
                {selectedSuburbs.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 mb-2">
                    {selectedSuburbs.map((suburb) => (
                      <span key={suburb} className="inline-flex items-center gap-1 pl-2.5 pr-1 py-1 bg-primary-900 text-white text-[11px] font-medium rounded-lg">
                        {suburb}
                ...
                <p className="text-[10px] text-stone-400 mt-1.5">Leave empty to search all suburbs</p>
              </div>
          ...
          <div className="space-y-5 min-w-0">
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                <h1 className="font-display text-2xl font-medium text-primary-900">
                  {mode === "live" ? "Best Homes for You" : "Top Investment Properties"}
                </h1>
                {totalMatches > 0 && (
              ...
              <div className="flex items-center gap-2 shrink-0">
                <div className="flex items-center gap-1.5">
                  <span className="text-[10px] font-semibold uppercase tracking-wider text-stone-400">Per page</span>
                  <select
                    value={pageSize}
            ...
            {searching && (
              <div className="space-y-4">
                {[1, 2, 3, 4, 5].map((i) => (
                  <div key={i} className="h-48 bg-white rounded-2xl border border-stone-200/70 animate-pulse" />
                ))}
              </div>
            )}
            ...
            {aiError && (
              <div className="p-4 bg-red-50 border border-red-100 rounded-xl text-red-600 text-sm">{aiError}</div>
            )}
            ...
            {aiLoading && (
              <div className="bg-white rounded-2xl border border-stone-200/70 shadow-card p-10 text-center">
                ...
                <p className="text-stone-400 text-xs mt-1">AI is classifying each property for {mode === "live" ? "living" : "investment"}</p>
              </div>
            )}
            ...
            {aiSuggestion && (
              <div className="bg-white rounded-2xl border border-stone-200/70 shadow-card overflow-hidden">
                <div className="flex items-center gap-3 px-7 py-5 border-b border-stone-100 bg-primary-900">
                  ...
                  <div>
                    <h2 className="font-display text-base font-semibold text-white">AI Property Report</h2>
                    <p className="text-white/50 text-xs">Deep analysis of top 20 properties · Gemini 2.5 Flash</p>
                  </div>
                  <button onClick={() => setAiSuggestion("")}
                    className="ml-auto text-white/40 hover:text-white/70 transition-colors text-lg leading-none">×</button>
                </div>
                <div className="p-7">
                  <div className="ai-markdown" dangerouslySetInnerHTML={{ __html: renderMarkdown(aiSuggestion) }} />
                </div>
